...
import {
    ...
    updatePrice,
    deletePrice
} from "../../../services/pricesApi"; // pastikan path sesuai

export const usePriceListByRole = (role) => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const data = await fetchPrices(role);
            setProdukList(
                data.map(p => ({
                    id: p.id,
                    nama: p.nama_produk,
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            await createPrice(payload, role);
